...
function setup(extra={}) {
  ...
  render(<ChallengePanel adapter={adapter} userId="a" />);return adapter;
}
it('shows the question before hint controls and hides hint text until explicitly opened',async()=>{
  ...
  const reveal=screen.getByRole('button',{name:'Reveal hint for sam'});
  ...
  expect(screen.getByText('Hint for Sam')).toBeTruthy();
  fireEvent.click(reveal); expect(screen.getByText('Subtract three for Sam.')).toBeTruthy();
  fireEvent.click(screen.getByRole('button',{name:'Hide hint for sam'}));
  expect(screen.queryByText('Subtract three for Sam.')).toBeNull();
