...
export function ChallengeEngine(props: ChallengeEngineProps) {
  ...
  return <ChallengePanel key={`${props.roomId}:${props.sessionId}:${props.userId}`} adapter={adapter} userId={props.userId} roomName={props.roomName} />;
}

export function ChallengePanel({ adapter, userId, roomName = 'Physics study circle', demo = false }: { adapter: ChallengeAdapter; userId: string; roomName?: string; demo?: boolean }) {
