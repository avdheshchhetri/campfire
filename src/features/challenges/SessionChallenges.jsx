...
export default function SessionChallenges({ sessionId }) {
  ...
  return <div className="cf-session-challenges">
    ...
    {!joined ? <section className="panel"><h2 className="font-display">Join the challenge circle</h2><p className="muted my-4">Each teammate joins on their own device. A round needs 1–6 participants.</p><button className="button" onClick={join} disabled={busy}>{busy ? 'Joining…' : 'Join session'}</button></section>
