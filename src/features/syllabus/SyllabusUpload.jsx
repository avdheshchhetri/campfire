...
function SyllabusForm({ roomId, onSaved }) {
  ...
  return <section className={cardClass}>
    <h2 className="font-display font-semibold text-2xl">Turn a syllabus into a study plan</h2>
    <p className="mt-2 text-sm leading-6 text-muted dark:text-stone-500">Upload a syllabus or short chapter, or paste its text. Review and edit the AI’s suggested topics before adding them.</p>
    <div className="mt-5 flex gap-2" role="group" aria-label="Syllabus input type">{[['text', 'Paste text'], ['pdf', 'Upload PDF']].map(([value, label]) => <button key={value} type="button" className={`${secondaryClass} ${mode === value ? 'bg-success-soft dark:bg-emerald-100' : ''}`} aria-pressed={mode === value} disabled={busy} onClick={() => { setMode(value); setTopics(null); setError(''); setMessage(''); }}>{label}</button>)}</div>
    <form className="mt-5 grid gap-4" onSubmit={event => { event.preventDefault(); void act(async () => {
      ...
      {mode === 'text' ? <><Field label="Syllabus text"><textarea className={inputClass} rows={8} required maxLength={50000} value={raw} onChange={e => { setRaw(e.target.value); setTopics(null); }} disabled={busy} placeholder="Unit 1: Data structures — arrays, linked lists, stacks…" /></Field><p className="text-xs text-muted dark:text-stone-500">{raw.length.toLocaleString()} / 50,000 characters</p></> :
        <div className="space-y-3 rounded-2xl border-2 border-dashed border-border dark:border-emerald-200 bg-success-soft/40 dark:bg-emerald-50/40 p-5">
          <Field label="Syllabus or chapter PDF"><input ref={fileInput} className={`${inputClass} file:mr-3 file:rounded-lg file:border-0 file:bg-success-soft dark:file:bg-emerald-100 file:px-3 file:py-2 file:text-success dark:file:text-emerald-900`} type="file" accept=".pdf,application/pdf" required disabled={busy} onChange={e => { setFile(e.target.files?.[0] || null); setTopics(null); setError(''); setMessage(''); }} /></Field>
          <p className="text-xs leading-5 text-muted dark:text-stone-600">Up to 200 MB and 1,000 pages. Large PDFs are automatically split into smaller batches. Each individual page must fit within 3 MB. Text and scanned PDFs are supported; scans must be readable. No password-protected files.</p>
          {file && <p className="break-all text-sm font-medium text-success dark:text-emerald-900">{file.name} · {(file.size / 1024 / 1024).toFixed(2)} MB</p>}
          <p className="text-xs leading-5 text-muted dark:text-stone-500">Analysis sends PDF batches to Gemini through our server. Large books take longer and use multiple AI requests; a desktop browser is recommended. Only the topics you save are stored in Campfire.</p>
        </div>}
