...
function TeachForm({ roomId, initialTopicId, onUpdated }) {
  ...
  return <section className={cardClass}>
    <p className="text-xs font-semibold uppercase tracking-widest text-emerald-700">Understanding, in your own words</p>
    <h2 className="mt-2 font-sans font-semibold text-2xl">Teach a topic</h2>
    <p className="mt-2 text-sm leading-6 text-stone-500">Explain it as you would to a teammate. The AI asks one follow-up before marking the topic verified.</p>
    ...
    {loading ? <p role="status" className="mt-4">Loading your topics…</p> : topics.length === 0 ? <p className="mt-4 text-sm">Add your syllabus before teaching a topic.</p> :
      <form onSubmit={submit} className="mt-5 grid gap-4">
        ...
        {followup && <div className="space-y-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-amber-800">Taught · One more connection</p><p className="font-sans font-semibold text-xl text-stone-800">{followup.question}</p><Field label="Your follow-up answer"><textarea autoFocus className={inputClass} rows={4} required maxLength={10000} value={answer} onChange={e => setAnswer(e.target.value)} disabled={busy} /></Field><p className="text-xs text-stone-500">Answer within 30 minutes. Reloading this page starts a new attempt.</p></div>}
        <ErrorMessage>{error}</ErrorMessage>
        {verdict && <div role="status" className={`rounded-xl border p-4 ${verdict.verified ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}><h3 className="font-semibold">{verdict.verified ? 'Understanding verified' : 'Keep building your explanation'}</h3><p className="mt-2 text-sm leading-6">{verdict.feedback}</p></div>}
        {selected?.status === 'verified' ? <p className="text-sm text-emerald-800">Your group has verified this topic. Pick another to continue.</p> : verdict ? <button type="button" className={buttonClass} onClick={reset}>Revise and try again</button> : <button className={buttonClass} disabled={busy || !selected || !explanation.trim() || (!!followup && !answer.trim())}>{busy ? 'Thinking with you…' : followup ? 'Check my understanding' : 'Ask my follow-up'}</button>}
