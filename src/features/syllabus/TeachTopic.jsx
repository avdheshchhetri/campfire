  ...
  return <section className={cardClass}>
    ...
    {loading ? <p role="status" className="mt-4">Loading your topics…</p> : topics.length === 0 ? <p className="mt-4 text-sm">Add your syllabus before teaching a topic.</p> :
      <form onSubmit={submit} className="mt-5 grid gap-4">
        ...
        {followup && <div className="space-y-4 rounded-xl border border-border dark:border-amber-200 bg-warning-soft dark:bg-amber-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-warning dark:text-amber-800">Taught · One more connection</p><div className="flex items-start gap-3"><p className="font-sans font-semibold text-xl text-primary dark:text-stone-800">{followup.question}</p><ReadAloud roomId={roomId} text={followup.question} mood="neutral" label="Read follow-up question aloud" /></div><Field label="Your follow-up answer"><textarea autoFocus className={inputClass} rows={4} required maxLength={10000} value={answer} onChange={e => setAnswer(e.target.value)} disabled={busy} /></Field><p className="text-xs text-muted dark:text-stone-500">Answer within 30 minutes. Your draft is kept in this browser tab when you navigate away.</p></div>}
        <ErrorMessage>{error}</ErrorMessage>
        {verdict && <div role="status" className={`rounded-xl border p-4 ${verdict.verified ? 'border-border dark:border-emerald-200 bg-success-soft dark:bg-emerald-50 text-success dark:text-emerald-900' : 'border-border dark:border-amber-200 bg-warning-soft dark:bg-amber-50 text-warning dark:text-amber-900'}`}><h3 className="font-display font-semibold">{verdict.verified ? 'Understanding verified' : 'Keep building your explanation'}</h3><div className="mt-2 flex items-start gap-3"><p className="text-sm leading-6">{verdict.feedback}</p><ReadAloud roomId={roomId} text={verdict.feedback} mood={verdict.verified ? "encouraging" : "neutral"} label="Read feedback aloud" /></div></div>}
