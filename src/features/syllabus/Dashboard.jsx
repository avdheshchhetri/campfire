...
const statuses = {
  untouched: { label: 'Untouched', color: 'border-red-200 bg-red-50 text-red-900', dot: 'bg-red-500' },
  taught: { label: 'Taught · needs verification', color: 'border-amber-200 bg-amber-50 text-amber-900', dot: 'bg-amber-500' },
  verified: { label: 'Verified', color: 'border-emerald-200 bg-emerald-50 text-emerald-900', dot: 'bg-emerald-500' },
};
...
function RoomDashboard({ roomId, onTeach, onAddSyllabus }) {
  ...
  return <section className="space-y-5">
    <header className={cardClass}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-emerald-700">{room?.subject || 'Your study circle'}</p><h1 className="mt-2 font-serif text-3xl">{room?.name || 'Room dashboard'}</h1><p className="mt-2 text-sm text-stone-500">{verified} of {topics.length} topics verified{room?.exam_date ? ` · Exam: ${room.exam_date}` : ''}</p></div><div className="text-right"><p className="text-xs text-stone-500">Invite your group</p><p className="mt-1 font-mono text-xl tracking-widest text-emerald-800">{room?.join_code || '…'}</p></div></div><progress aria-label="Verified topics" className="mt-5 h-2 w-full accent-emerald-700" max={Math.max(topics.length, 1)} value={verified} /></header>
