...
export default function CreateRoom({ onCreated = () => {} }) {
  ...
  return <section className={cardClass}>
    <p className="text-xs font-semibold uppercase tracking-widest text-success dark:text-emerald-700">Start a study circle</p>
    <h2 className="font-display mt-2 font-semibold text-2xl">Create a room</h2>
    {created ? <div className="mt-5 space-y-3" role="status"><p>{created.name} is ready. Share this code:</p><p className="font-mono text-3xl tracking-widest text-success dark:text-emerald-800">{created.join_code}</p></div> :
