  ...
  return (
    <div className="space-y-8">
      ...
      <div className="feature-grid">
        ...
        <section className="feature-card campfire-card"><Flame className="text-accent dark:text-[#b74724]" size={29} strokeWidth={1.5} /><span className="coming-soon">{activeSession ? 'Session available' : 'Ready to start'}</span><h2 className="font-display">The campfire</h2><p>Put your phones down and give your attention to the group.</p>{activeSession ? <Link className="button mt-7" to={`/room/${room.id}/session/${activeSession.id}`}>Open session<ArrowRight size={17} /></Link> : <div className="mt-7"><StartSessionButton roomId={room.id} onStarted={sessionId => navigate(`/room/${room.id}/session/${sessionId}`)} /></div>}</section>
