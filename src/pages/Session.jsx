...
export default function Session() {
  ...
  if (state.session?.is_active && !state.error) return <>
    <h1 className="page-heading mb-6">Your group’s focus space</h1>
    <Link className="button button-secondary mb-6" to={`/room/${roomId}`}><ArrowLeft size={17} />Back to room</Link>
    <SessionChallenges key={sessionId} sessionId={sessionId} />
  </>;
