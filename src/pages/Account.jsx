import { Auth0LoginButton, useIdentity } from '../features/auth/Auth0Login';
...
export default function Account() {
  ...
  return <section className="panel mx-auto max-w-xl p-7 space-y-6">
    ...
    {!user ? (identity.enabled ? <Auth0LoginButton /> : <AccountForm />) : <>
      <p className="muted">{profile?.auth0_id ? (identity.identity?.email || profile.display_name) : user.is_anonymous ? 'Guest profile — add an email to keep your rooms and progress across devices.' : user.email}</p>
      ...
      {profile?.auth0_id ? <p className="muted text-sm">Your sign-in is managed by Auth0.</p> : user.is_anonymous ? <form className="space-y-4" onSubmit={event => { event.preventDefault(); const email = new FormData(event.currentTarget).get('email'); run(async () => { await linkGuestEmail(email); setMessage('Check your email to verify this address. Then return to Account to set a password. Your rooms stay on this profile.'); }); }}>
      ...
      <button className="button button-secondary" disabled={busy} onClick={() => run(async () => { await (identity.enabled ? identity.logout() : logoutAccount()); })}>Sign out on this device</button>
    </>}
