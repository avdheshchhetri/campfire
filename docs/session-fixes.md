...
## Question-first quizzes and sleep continuity

Apply migration `20260913000500_quiz_assistance.sql` after all previous files. Questions appear directly under the topic; each teammate hint has an owner label/avatar and is hidden until clicked. Eight wrong submissions unlock the caller’s own hint and deduct one point once. Correct answers are tracked independently; individual rounds cannot be cancelled to skip ahead. All players must solve before the next quiz. Fresh Gemini generations require distinct canonical answers. Existing shared-answer rounds remain explicitly labeled as legacy/practice rounds; end one and generate a new individual round to use the new flow.

Sleep policy now deliberately retains the last saved orientation even after phone presence disconnects. A fresh up reading pauses focus; a fresh down reading does not. Offline down states are labeled as assumed focus. This also means a disconnected/closed phone may keep contributing until a new up reading or session end; it is not verified background sensor tracking. The shared display still requires its own database connection.

Phone navigation now uses the existing authenticated identity instead of a second sign-in check and isolates rendering failures behind a retry panel. Local phone navigation succeeds; the reported intermittent blank page was not reproduced. Device-specific production verification remains necessary.

Validation: 133 automated tests passed, including per-user penalties, hidden hints, next-round gating, phone sleep transitions, and feature retry recovery. TypeScript and the production build passed. Local phone navigation and light-mode contrast were inspected. Live Gemini verification hit a timeout followed by quota/rate limiting; no successful new live generation is claimed for this update.
