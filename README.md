...
### Games usability update (migration 008)

Apply [20260913000800_game_controls.sql](supabase/migrations/20260913000800_game_controls.sql) after 007, then deploy the latest app. Spark Round is now untimed: all votes reveal the result, or a participant can end the question early. Review the answer/explanation, then select Next question. End game for everyone closes any game and retains its scores. Only the selected start button shows Preparing. Games, Flashcards and join/sign-in panels use inner padding. Flashcards generate one topic at a time with simpler Gemini JSON requests, preserving successful topics if a later request fails.
