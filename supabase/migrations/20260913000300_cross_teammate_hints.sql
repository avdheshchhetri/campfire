-- Pass each teammate’s hint to the next participant. Existing rounds are unchanged.
begin;
...
begin
 ...
 -- assignments rotate each named hint to the next teammate, never exposing the answer.
 ...
 for i in 1..greatest(n,clue_count) loop
  ...
  values(cid,roster[(i%n)+1],concat(case when n=1 then 'Solo hint' else 'Hint for ' || coalesce((select display_name from public.profiles where id=roster[((i-1)%n)+1]), 'your teammate') end, E'\n\n',clue_texts[((i-1)%clue_count)+1]),true,i-1);
 end loop;
 return cid;
end;
$$;
